...
fetch(url + encodeURI('tblEmployees?filter={"where": {"supervisorEmployeeId": 2}}'))
.then(
  function(response) {
    if (response.status !== 200) {
      console.log('Looks like there was a problem. Status Code: ' +
    ...
    response.json().then(function(data) {
      var nomineeString = "";
      data.forEach((el : any) => nomineeString = nomineeString + "{\"nominee\": \"" +  el.id + "\"}");
      
      fetch(url + encodeURI('Nominations?filter={"where": {"or":[' + nomineeString + ']}}'))
        .then(
          function(response) {
            if (response.status !== 200) {
              console.log('Looks like there was a problem. Status Code: ' +
